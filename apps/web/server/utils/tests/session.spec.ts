...
describe('session', () => {
  it('verifica access token válido com sub e role', () => {
    const token = signAccessToken({ sub: 'user-id', role: USER_ROLE.VIEWER }, 900);
  ...
  it('lê sessão do cookie access_token', () => {
    const token = signAccessToken({ sub: 'cookie-user', role: USER_ROLE.VIEWER }, 900);
  ...
  it('detecta sessão expirada', () => {
    const token = signAccessToken({ sub: 'user-id', role: USER_ROLE.VIEWER }, -10);
    const session = verifyAccessToken(token, JWT_SECRET);
